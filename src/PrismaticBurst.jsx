...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    // Force WebGL2 for GLSL #version 300 es
    const gl2 = canvas.getContext('webgl2', { alpha: true, antialias: false, premultipliedAlpha: false, depth: false, stencil: false });
    if (!gl2) {
      console.warn('WebGL2 not available; PrismaticBurst requires WebGL2.');
      return () => {
        try { container.removeChild(canvas); } catch (e) { void e; }
      };
    }

    const renderer = new Renderer({ dpr, context: gl2 });
